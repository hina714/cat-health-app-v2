...
PostgreSQL のホスティングをはじめ、バックエンドに必要な機能をまとめて提供してくれます。

### なぜ Supabase を使うのか？

**1. DB サーバーを自分で用意しなくていい**
通常は PostgreSQL サーバーを自分でセットアップ・管理する必要がありますが、Supabase がホスティングを肩代わりしてくれます。API は引き続き Next.js の Route Handlers で自前実装します。

**2. PostgreSQL が使える**
SQL が使えるので、複雑なデータ操作が得意です。
