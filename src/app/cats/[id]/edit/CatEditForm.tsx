...
export default function CatEditForm({ cat }: Props) {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    if (file.size > MAX_FILE_SIZE) { setError('画像は10MB以下にしてください'); return }
    setError('')
  ...
  async function handleCropConfirm() {
    ...
    setIconData(await cropToBase64(imageSrc, croppedArea))
    setImageSrc(null)
  }
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} className={styles.form}>
        ...
        <div className={styles.cropperContainer}>
          ...
          <div className={styles.zoomRow}>
            ...
            <input type="range" min={1} max={3} step={0.01} value={zoom}
              onChange={(e) => setZoom(Number(e.target.value))} className={styles.zoomSlider} />
          </div>
          <div className={styles.cropButtons}>
            <button type="button" onClick={() => setImageSrc(null)} className={styles.btnCancel}>キャンセル</button>
            <button type="button" onClick={handleCropConfirm} className={styles.btnPrimary}>この写真を使う</button>
          </div>
        </div>
      )}
      ...
      <div className={styles.actions}>
        <button type="button" onClick={() => router.push('/')} className={styles.btnCancel}>キャンセル</button>
